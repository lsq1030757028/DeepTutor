...
| 6 | `web/locales/en/app.json` + `zh/app.json` | 成对追加文案 key。`i18n:parity` 是硬闸，两边 key 集必须一致。<br>**2026-08-11（M2）：+2 键**（`Test Journey` / `Test Journey tooltip`）。<br>**2026-08-12：+155 键，合 main 阻断项已清**。M2 薄壳八屏与富卡四卡的中文硬编码全部外提（129 条字符串字面量 + 54 处 JSXText → 0），该处 eslint warning 由 47 降到 0。<br>**判据不看 warning 数**：`eslint/i18n-plugin.mjs` 只挂 `JSXText`/`JSXAttribute`，`copy.ts`(44) / `client.ts`(8) / `types.ts`(4) 这 56 条硬编码它一条都报不出——"warning 没涨"证明不了没硬编码。改为逐个枚举源码中文串 + 逐个核对引用键在 en/zh 都有真值，落在 `web/tests/test-journey-i18n.test.ts`（回归闸第 3 层真跑）。 | 追加行在文件尾部，merge 时保留我方追加段，其余取上游 | **否**——文案库是集中式的 |
...
| 9 | `web/app/(workspace)/home/[[...sessionId]]/page.tsx` | `CAPABILITIES` 常量数组加 1 项。DT 没有意图路由，模式**由用户在 picker 里显式选中**，不在这张表里它就不存在。<br>**2026-08-12：该常量加 `export`（同一行，不新增登记行）**。原本写在组件内部未导出，python 侧与 node 侧都断不到 —— 本触点此前**零机械守**，漏改的表现是入口直接消失且没有任何测试会红。导出后判据落在 `web/tests/capability-picker.test.ts`：import 真数组喂真 `ChatComposer` 渲染展开态 picker，断「测试」在一级菜单（不在 More 浮层），并带一条摘掉该项必红的反例。<br>Next 的 page 校验是 `Specific extends AppPageConfig<...>`（结构性、非精确），多一个具名导出合法；`npx next build` 实跑通过。<br>**残余缺口**：page 是否真把这张表传给 composer 未被断言（要渲染整个 page，依赖 router/context/网络），须走浏览器级 e2e 才能补。 | 保留这一项与 `export`，其余取上游 | **否** |
| 10 | `web/components/chat/home/ChatMessages.tsx` | 富结果卡挂点 5 处：dynamic import、模式徽章分支、extractor useMemo、默认渲染抑制、渲染分支。<br>**⚠ 这是登记表的首个「第三类触点」——改组件内部逻辑，而不是往集中式清单里加一行。一条分界线在此被跨过。**<br>**改动原因栏如实写：改渲染逻辑（含事件流累加）**，不写「加一行」。那个 useMemo 依赖 `msg.events` 而非 `resultEvent`（两张卡是流式的），因此多一条流式↔落地一致性契约、多一个卡序列渲染语义——行数与登记行不变，**耦合面变宽**。<br>**状态：P3 候选已落地，合入时须保留完整 5 处挂点。** | 同步时**必须人读上游 diff**，不能机械取舍。上游若重构卡片分派结构，这 5 处需整体重写 | **否** |
| 11 | `deeptutor/i18n/metadata_i18n.py` | `_CAPABILITY_DESCRIPTIONS` 加 1 项 `"test"`（picker 那一行显示的中英文案） | 保留这一项，其余取上游 | **否** |
| 12 | `.github/workflows/tests.yml` | 去掉 `push`/`pull_request` 自动触发，只留 `workflow_dispatch`。私有仓 Actions 额度会烧穿必过闸（决策 0034） | 同步上游时保留 `on: workflow_dispatch`，其余 jobs 取上游 | **否** |
...
当前触点已有三类：**集中式注册表**（导航数组、路由注册）、**CI 工作流的仓库身份**、
以及 `ChatMessages.tsx` 的**组件内部渲染逻辑**。前两类多为行级追加；第三类不再天然 merge 友好，
每次上游同步必须人工阅读双方 diff，不能靠机械取舍。
