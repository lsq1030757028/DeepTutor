...
> ⚠️ **本落点已被 [0012](0012-tapd-into-container-stdio.md) 取代（2026-08-08）。**
> 下面这段的前提是错的：`mcp-server-tapd` 在 streamable-http 模式下**不读 HTTP header**，
> 令牌只能是进程环境变量，所以「令牌就是条目上的一个 header」在技术上不成立。
> 本节勘察表里其实同时列了「环境变量」编辑器——落点一挑错了那一个。
> 结论方向（走平台机制、作废自建配置区）仍然成立，改由 0012 的「进容器 + stdio」实现。
